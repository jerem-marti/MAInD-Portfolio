...
// shows "measuring…" rather than another page's number.
const co2 = ref<number | null>(null)
const cleanerThanPct = ref<number | null>(null)
const loaded = computed(() => co2.value !== null && cleanerThanPct.value !== null)
...
// --- Live per-page fetch (client only) --------------------------------------
// One call per URL per day, cached in localStorage — same behavior as the
// official Website Carbon badge. /b returns { c: grams, p: percentile }.
const CACHE_TTL = 864e5 // 24h
  ...
  try {
    ...
    if (typeof data?.c === 'number' && typeof data?.p === 'number') {
      ...
      try {
        ...
        // storage full/blocked → value still shows this session
      }
    }
    // else: error payload (un-indexed page) → stay in "measuring" state.
  } catch {
...
<template>
  <div class="col-span-12 border-t border-brand-hairline pt-6">
    <p class="flex items-center gap-2 font-mono uppercase tracking-[0.08em] text-[11px] text-brand-ink-muted">
      ...
      <span>
        <template v-if="loaded">{{ t('footer.carbon.line', { co2: co2Display, percent: percentDisplay }) }}</template>
        <template v-else>{{ t('footer.carbon.measuring') }}</template>
